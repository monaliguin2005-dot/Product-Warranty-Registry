...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  const [warrantyData, setWarrantyData] = useState<{ owner: string; expiry_date: string } | null>(null);

  const truncate = (addr: string) => `${addr.slice(0, 6)}...${addr.slice(-4)}`;

  const handleRegister = useCallback(async () => {
    if (!walletAddress) return setError("Connect wallet first");
    if (!regProductId.trim() || !regOwner.trim() || !regExpiry.trim()) return setError("Fill in all fields");
    setError(null);
    setIsRegistering(true);
    setTxStatus("Awaiting signature...");
    try {
      await registerWarranty(walletAddress, regProductId.trim(), regOwner.trim(), regExpiry.trim());
      setTxStatus("Warranty registered on-chain!");
      setRegProductId("");
      setRegOwner("");
      setRegExpiry("");
  ...
  }, [walletAddress, regProductId, regOwner, regExpiry]);
  ...
  const handleLookup = useCallback(async () => {
    ...
    try {
      const result = await getWarranty(lookupId.trim(), walletAddress || undefined);
      if (result && Array.isArray(result) && result.length >= 2) {
        setWarrantyData({ owner: result[0], expiry_date: result[1] });
      } else {
        ...
        <AnimatedCard className="p-0" containerClassName="rounded-2xl">
          ...
          <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-white/90">Product Warranty Registry</h3>
          ...
          <div className="p-6">
            ...
            {activeTab === "lookup" && (
              <div className="space-y-5">
                <MethodSignature name="get_warranty" params="(product_id: String)" returns="-> (String, String)" color="#4fc3f7" />
                ...
                <ShimmerButton onClick={handleLookup} disabled={isLookingUp} shimmerColor="#4fc3f7" className="w-full">
                  {isLookingUp ? <><SpinnerIcon /> Looking up...</> : <><SearchIcon /> Lookup Warranty</>}
                </ShimmerButton>

                {warrantyData && (
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden animate-fade-in-up">
                    <div className="border-b border-white/[0.06] px-4 py-3 flex items-center justify-between">
                      <span className="text-[10px] font-medium uppercase tracking-wider text-white/25">Warranty Details</span>
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-mono text-sm text-white/80">{warrantyData.owner}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-white/35">Expiry Date</span>
                        <span className="font-mono text-sm text-white/80">{warrantyData.expiry_date}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {activeTab === "register" && (
              <div className="space-y-5">
                <MethodSignature name="register_warranty" params="(product_id: String, owner: String, expiry_date: String)" color="#7c6cf0" />
                <Input label="Product ID" value={regProductId} onChange={(e) => setRegProductId(e.target.value)} placeholder="e.g. PROD-001" />
                <Input label="Owner Name" value={regOwner} onChange={(e) => setRegOwner(e.target.value)} placeholder="e.g. John Doe" />
                <Input label="Expiry Date" value={regExpiry} onChange={(e) => setRegExpiry(e.target.value)} placeholder="e.g. 2025-12-31" />
                {walletAddress ? (
                  <ShimmerButton onClick={handleRegister} disabled={isRegistering} shimmerColor="#7c6cf0" className="w-full">
                    {isRegistering ? <><SpinnerIcon /> Registering...</> : <><PackageIcon /> Register Warranty</>}
                  </ShimmerButton>
                ) : (
                  <button
                    onClick={onConnect}
                  ...
                  >
                    Connect wallet to register warranty
                  </button>
                )}
              </div>
            )}
          ...
          <div className="border-t border-white/[0.04] px-6 py-3 flex items-center justify-between">
            <p className="text-[10px] text-white/15">Product Warranty Registry &middot; Soroban</p>
            <div className="flex items-center gap-2">
              {["Active", "Expired"].map((s, i) => (
                <span key={s} className="flex items-center gap-1.5">
                  <span className={cn("h-1 w-1 rounded-full", s === "Active" ? "bg-[#34d399]" : "bg-[#f87171]")} />
